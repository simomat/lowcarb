...
export const setListElements = listContent =>
    getRootElem().map(rootElem => returnTrue(rootElem.innerHTML = Sanitizer.unwrapSafeHTML(listContent)));

export const onListItemClick = handler =>
    getRootElem().map(rootElem => rootElem.addEventListener('click', event => handler(event.target)));