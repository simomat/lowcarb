...
const extractSettings = storage => Maybe.of(storage.settings).orElse(getDefaultSettings);
const loadSettings = () => getStorage('settings').map(extractSettings);
...
export const ifRemoveCookiesOnStartup = () => loadSettings().map(settings => settings.removeOnStartup);
export const setRemoveCookiesOnStartup = value => saveSetting('removeOnStartup', value);
export const ifNotifyOnRemovedCookies = () => loadSettings().map(settings => settings.notifyCookiesRemoved);
export const setNotifyCookiesRemoved = value => saveSetting('notifyCookiesRemoved', value);
